...
def main():
    ...
    try:
        ...
        # Training loop - just do one iteration for debugging
        result = algo.train()
        ...
        worker_manager = algo.workers

        if NUM_ROLLOUT_WORKERS == 0:
            logger.info("Signaling local worker's environment to initiate graceful shutdown (NUM_ROLLOUT_WORKERS == 0).")
            try:
                # For local worker, EnvRunnerGroup.local_worker() gives the LocalEnvRunner
                local_env_runner = worker_manager.local_worker()
                if local_env_runner:
                    # The actual environment instance is often under an 'env' attribute or similar
                    # For SingleAgentEnvRunner, it might be env_runners[0].env
                    # For LocalEnvRunner, it might be local_env_runner.env or local_env_runner.get_sub_environments()[0]
                    # Let's try a common pattern; this might need adjustment based on actual LocalEnvRunner structure
                    actual_env = None
                    if hasattr(local_env_runner, 'env'): # Common for RolloutWorker
                        actual_env = local_env_runner.env
                    elif hasattr(local_env_runner, 'get_sub_environments') and callable(local_env_runner.get_sub_environments):
                        sub_envs = local_env_runner.get_sub_environments()
                        if sub_envs:
                            actual_env = sub_envs[0]
                    
                    if actual_env and hasattr(actual_env, 'signal_graceful_shutdown'):
                        actual_env.signal_graceful_shutdown()
                        logger.info("Signal sent to local environment.")
                    elif actual_env:
                        logger.warning("Local environment does not have 'signal_graceful_shutdown' method.")
                    else:
                        logger.warning("Could not retrieve the actual local environment instance.")
                else:
                    logger.warning("Local worker instance not found via worker_manager.local_worker().")
            except Exception as e_local_signal:
                logger.error(f"Error signaling local worker's environment: {e_local_signal}", exc_info=True)
            ...
            try:
                worker_manager.foreach_env(lambda env: env.signal_graceful_shutdown())
                logger.info("Signal sent to all remote environments via foreach_env.")
        ...
        for i in range(GRACEFUL_SHUTDOWN_DRAIN_ITERATIONS):
            ...
            if (datetime.datetime.now() - drain_start_time).total_seconds() > GRACEFUL_SHUTDOWN_TIMEOUT_SECONDS:
                logger.warning(f"Graceful shutdown drain timeout ({GRACEFUL_SHUTDOWN_TIMEOUT_SECONDS}s) reached.")
                break
